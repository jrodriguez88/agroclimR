...
function(df, fill=NA_real_, IQR_times = 5, max_temp_diff = 15, by_var = T){
  ...
  if (is.Date(df$date) & all(c("tmax", "tmin", "rain") %in%  var_names)  & !isTRUE(by_var)){
    ...
    } else if (is.Date(df$date) & any(str_detect(var_names, "rain|prec"))) {
    ...
    } else if (is.Date(df$date) & any(str_detect(var_names, "tmax")) & isTRUE(by_var)) {
    ...
    } else if (is.Date(df$date) & any(str_detect(var_names, "tmin")) &  isTRUE(by_var) ) {
