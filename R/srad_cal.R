...
function(data, lat, sh_name =  "sbri", A = 0.29, B = 0.45, kRs = 0.175, fill = F){
  ...
  if (sh_name %in% colnames(data)){
    ...
    data <- data %>%
      mutate(
        extraT = sirad::extrat(lubridate::yday(date), lat_rad)$ExtraTerrestrialSolarRadiationDaily, # Calcula la radiacion extraterrestre
        srad = sirad::ap(date, lat = lat, lon = lon,    # aqui aplica Angstrom-Prescott
                  extraT, A, B, sbri))
  ...
  if (isTRUE(fill)) {
    ...
    data <- data %>%
      mutate(
        ...
        srad = if_else(srad>max_srad|srad<0|is.na(srad),  median(data$srad, na.rm = T), srad))
